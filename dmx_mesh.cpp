...
// runtime proof that the IOCP->io_uring pump wiring works end-to-end (LinuxPortPlan §9,
// Phase 3 — "PipeMeshTest-analog on Dmx").
//
...
protected:
                                     ...
                                     P2Psize_t   iSize) override
    {
        ...
        {
            std::printf("[CLIENT] Login ack from '%s' - Dmx connection ready.\n",
...
{
    std::printf("=== dmx_mesh - single-process two-hub in-process Dmx probe ===\n");
    std::printf("Service : %s\n\n", N(kServiceName).c_str());
    std::fflush(stdout);
    ...
    // (No WSAStartup: the Dmx transport uses no sockets.)
    ...
    Log("SERVER", "service Dmx connection posted");

    // Let the server pump run Listen()/Accept() before the client connects.
    Sleep(750);
    ...
    Log("CLIENT", "client Dmx connection posted");
    ...
    {
        Log("MAIN", "SUCCESS - server received the client's BCast over Dmx");
        nExit = 0;
    }
    else
    {
