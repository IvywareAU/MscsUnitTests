...
// runtime proof of the socket transport (accept/connect/recv/send) over io_uring on
// Linux (LinuxPortPlan §9, Phase 3 — "AlexTest green Linux<->Linux", single-process form).
//
...
protected:
                                     ...
                                     P2Psize_t   iSize) override
    {
        ...
        {
            std::printf("[CLIENT] Login ack from '%s' - loopback TCP connection ready.\n",
...
{
    std::printf("=== wsa_mesh - single-process two-hub loopback-TCP probe ===\n");
    std::printf("Port : %d (127.0.0.1)\n\n", (int)kTestPort);
    std::fflush(stdout);
    ...
    // ---- Hub A: SERVER (binds INADDR_ANY + listens on the loopback port) --
    ...
    Log("SERVER", "service TCP connection posted (bind INADDR_ANY + listen)");

    // Let the server pump bind + listen before the client dials.
    Sleep(750);
    ...
    Log("CLIENT", "client TCP connection posted (connect 127.0.0.1)");
    ...
    {
        Log("MAIN", "SUCCESS - server received the client's BCast over loopback TCP");
        nExit = 0;
    }
    else
    {
